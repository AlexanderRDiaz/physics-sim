##########################################
# JES Media Functions, Version 19
#
...
class JESSample:
    ...
    def __str__(self):
        return (
            ...
            + str(self.index)
            + ' with value '
            + str(self.JESsnd.samples[self.index])
        )
...
class JESImage:
    ...
    def __str__(self):
        return (
            ...
            + self.filename
            + '\n   height '
            + str(self.PILimg.height)
            + ' width '
            + str(self.PILimg.width)
        )
...
# thanks to Andrew Sauer
def copyInto(smJESimg, lgJESimg, startX, startY):
    if smJESimg.PILimg.mode == 'RGBA':
        lgJESimg.PILimg.paste(smJESimg.PILimg, (int(startX), int(startY)), smJESimg.PILimg)
    else:
        lgJESimg.PILimg.paste(smJESimg.PILimg, (int(startX), int(startY)))
...
def addRect(JESimg, startX, startY, width, height, color=(0, 0, 0)):
    d = ImageDraw.Draw(JESimg.PILimg, mode='RGB')
    d.rectangle(
        [int(startX), int(startY), int(startX + width), int(startY + height)],
        outline=color,
        fill=None,
    )
...
def writeSlideShowTo(fileName, delay=1):
    ...
    writeAnimatedGif(images, fileName, 1.0 / delay) # pyright: ignore
...
# Version 19 8/16/24
#
# Modified copyInto to handle transparent backgrounds
#
